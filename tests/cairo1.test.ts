...
describeIfDevnet("Cairo 1 Devnet", () => {
  describe("API &  Contract interactions", () => {
    ...
    test("Cairo1 simple getStorageAt variables retrieval", async () => {
      ...
      const storage2 = await account.getStorageAt(
        cairo1Contract.address,
        key + 1n,
      );
    ...
    test("Cairo 1 Contract Interaction - echo flat un-nested Array u8, uint256, bool", async () => {
      ...
      const status2 = await cairo1Contract.echo_array_bool([
        true,
        true,
        false,
        false,
    ...
    test("myCallData.compile for Cairo 1", async () => {
      ...
      const contractCallData: CallData = new CallData(
        compiledComplexSierra.abi,
      );
      const callDataFromObject: Calldata = contractCallData.compile(
        "constructor",
        myRawArgsObject,
      );
      const callDataFromArray: Calldata = contractCallData.compile(
        "constructor",
        myRawArgsArray,
      );
