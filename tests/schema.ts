...
import * as accountSchemas from "./schemas/account.json";
import * as libSchemas from "./schemas/lib.json";
import * as providerSchemas from "./schemas/provider.json";
import * as rpcSchemas from "./schemas/rpc.json";
import * as sequencerSchemas from "./schemas/sequencer.json";
  ...
  expect.extend({
    toMatchSchemaRef(received: object, name: string) {
      const schema = schemas.find((s) =>
        Object.keys(s.definitions).includes(name),
      );
